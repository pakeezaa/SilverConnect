...
# Shared list of tech-help skills used by registration, the helpers filter,
# and the volunteer profile-edit form.
SKILL_OPTIONS = ["Smartphones", "Wi-Fi Setup", "Video Calls", "Email", "Tablets",
                  "Social Media", "Online Banking", "Streaming"]
...
def helpers():
    ...
    return render_template("helpers.html", volunteers=volunteers, neighborhoods=neighborhoods,
                           all_skills=SKILL_OPTIONS, selected_neighborhood=neighborhood,
...
def register_volunteer():
    ...
    if request.method == "POST":
        ...
        if not name or not email or not neighborhood or not password:
            ...
            session["name"] = name
            return redirect(url_for("profile"))
    return render_template("register_volunteer.html", error=error, all_skills=SKILL_OPTIONS)
